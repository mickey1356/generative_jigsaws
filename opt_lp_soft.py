...
def precompute_indices(V, E, iV_pin, device="cuda"):
    nV = V.shape[0]
...
def plot_mesh(V, F, filename):
    plt.figure(dpi=300)
    plt.xlim(-1, 1)
    plt.ylim(-1, 1)
    plt.triplot(V[:, 0], V[:, 1], F, color="black", lw=0.5)
    plt.gca().invert_yaxis()
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig(filename)
    plt.close()

def smooth_boundary_loss(V, bnd_idx):
    V_bnd = V[bnd_idx]
    V_bnd_prev = torch.roll(V_bnd, shifts=1, dims=0)
    V_bnd_next = torch.roll(V_bnd, shifts=-1, dims=0)
    e_n = V_bnd_next - V_bnd
    e_p = V_bnd - V_bnd_prev
    
    # lengths
    len_n = torch.norm(e_n, dim=1).clamp_min(1e-8)
    len_p = torch.norm(e_p, dim=1).clamp_min(1e-8)
    len_avg = 0.5 * (len_n + len_p)
    ...
    # curvature vector
    k_vec = (t_n - t_p) / len_avg[:, None]
    
    # minimize squared curvature
    loss_per_vertex = (k_vec.pow(2).sum(dim=1)) * len_avg
    loss = loss_per_vertex.sum()
    return loss

def triangle_area_loss(V, F):
    # minimize the variance of the triangles
    A = 0.5 * torch.norm(torch.cross(V[F[:, 1]] - V[F[:, 0]], V[F[:, 2]] - V[F[:, 0]], dim=1), dim=1)
    return A.var()
...
if __name__ == "__main__":
    ...
    os.makedirs(f"{FOLDER}/meshes", exist_ok=True)
    os.makedirs(f"{FOLDER}/splits", exist_ok=True)
    ...
    V, F = mesher.circle_in_square(N=32, r=0.8, s=[-1, 1], e=0.05)
    nV = V.shape[0]
    E = igl.edges(F)
    ...
    M_r, M_c, C_d, rhs = precompute_indices(V, E, iV_pin, device=DEVICE)
    ...
    # simple split, top half and bottom half
    # for now, do manual splits
    # F_shape_centers = np.mean(V[F[fids]], axis=1)
    # F_split_A = np.where(F_shape_centers[:, 0] < 0)[0]
    # F_split_B = np.where(F_shape_centers[:, 0] >= 0)[0]
    ...
    bnd_shape = igl.boundary_loop(F[fids])
    bnd_shapes = [bnd_shape]
    ...
    # center shape and add third dimension of zeros
    def center_shape(V_n, iV_shape, optimize_center=False):
        # center the shape without backprop
        V_shape = V_n[iV_shape]
        shape_center = V_shape.mean(dim=0)
        if not optimize_center:
            shape_center = shape_center.detach()
        V_centered = V_n - shape_center[None, :]
        return torch.hstack([V_centered, torch.zeros((V_centered.shape[0], 1), device=DEVICE)])

    # convert to homogeneous coordinates and batch
    def convert_to_homogeneous(V):
        V_hom = torch.hstack([V, torch.ones((V.shape[0], 1), device=DEVICE)])[None, ...]
        return V_hom
    ...
    for it in tqdm.trange(ITERATIONS):
        ...
        V_n = compute_V_new(x)
        # make sure the shape remains within bounds
        # i.e. clip the vertex coordinates to [-1, 1]
        V_n = torch.clamp(V_n, -1, 1)

        imgs = []
        losses = []

        # render images for sds image losses
        for ind, (s_faces, s_iv, s_text, s_wt) in enumerate(zip(F_shapes, iV_shapes, embeds, WEIGHTS)):
            # we only want to optimize the full shape center (id = 0)
            V_3d = center_shape(V_n, s_iv, ind==0)

            # # we want to minimize the variance of triangle areas
            # if ind == 0:
            #     area_loss = triangle_area_loss(V_3d, s_faces)
            #     losses.append((area_loss, 0.5))
            
            V_hom = convert_to_homogeneous(V_3d)
            
            # render image
            img = render(V_hom, s_faces, black, bgs, glctx, RES)
            imgs.append(img[0].detach().cpu().numpy())

            # sds loss
            losses.append((guidance(img, s_text)["loss_sds"], s_wt))

        # for ind, bnd in enumerate(bnd_shapes):
        #     bnd_loss = smooth_boundary_loss(V_n, bnd)
        #     losses.append((bnd_loss * 0.1, 2))
        ...
        with torch.no_grad():
            if it % 50 == 0:
                V_3d = center_shape(V_n, iV_shapes[0])
                V_hom = convert_to_homogeneous(V_3d)
                ...
                plt.imsave(f"{FOLDER}/{it}.png", imgs[0])

                V_hom_np = V_hom[0].detach().cpu().numpy()
                F_np = F_shape.detach().cpu().numpy()
                plot_mesh(V_hom_np, F_np, f"{FOLDER}/meshes/{it}.png")
    ...
    with torch.no_grad():
        ...
        V_hom_np = V_hom[0].detach().cpu().numpy()
        F_np = F_shape.detach().cpu().numpy()
        plot_mesh(V_hom_np, F_np, f"{FOLDER}/mesh_final.png")
